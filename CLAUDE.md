...
- JavaScript confirm dialogs that include user-supplied strings must use `| tojson`: `return confirm('Delete ' + {{ value | tojson }} + '?')`. Plain `{{ value }}` interpolated into a JS string is stored XSS.
- Rate limiting via `@limiter.limit('10 per minute')` (imported from `app`) is applied to `auth.login` (10/min; 30/hour) and both password reset routes (`members.reset_password`, `admin.reset_user_password`).
